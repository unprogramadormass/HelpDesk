...
// 1. ACTIVAR ERRORES TEMPORALMENTE (Cambiaremos esto a 0 cuando el sistema esté listo)
...
// 4. CONSULTA EXPANDIDA DEL TICKET (CORREGIDA: Toma Área y Sucursal del TICKET)
$sqlTicket = "SELECT t.*, 
    -- DATOS DEL TICKET (Sucursal y Área a la que pertenece el ticket)
...
if (!$resultTicket) {
    die("Error en la consulta del ticket: " . $conn->error);
}
...
// 4.5. EXTRAER EVIDENCIAS (JSON a Arreglo de PHP)
$evidencias = [];
...
if (!$historial) {
    die("Error obteniendo el historial: " . $conn->error);
}
...
<head>
    ...
    <title>Ticket Details <?php echo $ticket['folio']; ?> | HelpDesk</title>
...
<body class="bg-slate-50 text-slate-800 dark:bg-darkbg dark:text-slate-200 min-h-screen flex flex-col theme-trans">
    ...
    <main class="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm p-6 border border-slate-100 dark:border-slate-700">
                ...
                <div class="prose dark:prose-invert max-w-none text-sm text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
            ...
            <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                        <i class="fas fa-paperclip text-slate-400"></i> Evidencias Adjuntas
                    </h3>
                </div>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    <?php if(empty($evidencias)): ?>
                        <div class="p-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg text-center text-slate-400 text-xs col-span-full">
                    ...
                    <?php else: ?>
                        <?php foreach($evidencias as $ev): 
                            ...
                            // Configuración dinámica para Fancybox
                            $fancyboxAttr = "";
                            $target = "";
                            ...
                            if(in_array($ext, ['jpg','jpeg','png','gif','webp'])) {
                                $icon = 'fas fa-image text-blue-500';
                                $fancyboxAttr = 'data-fancybox="gallery" data-type="image"';
                                $target = "";
                                $actionText = "Vista Previa";
                                $actionIcon = "fas fa-eye";
                            } else if($ext == 'pdf') {
                                $icon = 'fas fa-file-pdf text-red-500';
                                $fancyboxAttr = 'data-fancybox="gallery" data-type="pdf"';
                                $target = "";
                                $actionText = "Ver PDF";
                                $actionIcon = "fas fa-eye";
                            } else if(in_array($ext, ['mp4','avi','mov','webm'])) {
                                $icon = 'fas fa-file-video text-purple-500';
                                $fancyboxAttr = 'data-fancybox="gallery" data-type="video"';
                                $target = "";
                        ...
                        ?>
                        <a href="<?php echo $fileUrl; ?>" <?php echo $fancyboxAttr; ?> target="<?php echo $target; ?>" class="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-lg shadow-sm hover:border-blue-400 dark:hover:border-blue-500 transition group relative overflow-hidden cursor-pointer">
                            <i class="<?php echo $icon; ?> text-2xl group-hover:scale-110 transition-transform"></i>
                            <div class="flex flex-col min-w-0">
                                <span class="truncate text-xs font-bold text-slate-700 dark:text-slate-300" title="<?php echo htmlspecialchars($ev); ?>">
                                    <?php echo htmlspecialchars($ev); ?>
                                </span>
                                <span class="text-[10px] text-slate-400 uppercase mt-0.5 group-hover:text-blue-500 transition-colors">
                                    <i class="<?php echo $actionIcon; ?> mr-1"></i><?php echo $actionText; ?>
                                </span>
                            </div>
                        </a>
                        <?php endforeach; ?>
            ...
            <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col overflow-hidden">
                ...
                <div class="p-6 space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar">
                    <?php while($h = $historial->fetch_assoc()): ?>
                        <?php 
                            $avt = $h['resp_avatar'] ? "/HelpDesk/assets/img/avatars/".$h['resp_avatar'] : "https://api.dicebear.com/7.x/avataaars/svg?seed=".$h['resp_nombre'];
                            $date = date("d M, h:i A", strtotime($h['fecha_movimiento']));
                        ?>
                        <?php if($h['tipo_movimiento'] === 'Comentario'): ?>
                            <div class="flex gap-4 animate-fade-in">
                                <img src="<?php echo $avt; ?>" class="w-10 h-10 rounded-full border border-slate-200 bg-white shrink-0">
                                <div class="flex-1">
                                    <div class="flex justify-between items-baseline mb-1">
                                        <span class="font-bold text-sm text-slate-700 dark:text-white"><?php echo $h['resp_nombre']; ?></span>
                                        <span class="text-xs text-slate-400"><?php echo $date; ?></span>
                                    </div>
                                    <div class="bg-slate-50 dark:bg-slate-800 p-3 rounded-br-xl rounded-bl-xl rounded-tr-xl border border-slate-100 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300">
                        ...
                        <?php else: ?>
                            <div class="flex items-center gap-4 justify-center py-2 opacity-60">
                                ...
                                <span class="text-[10px] uppercase font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">
                                    <?php echo $h['descripcion_evento']; ?> • <?php echo $date; ?>
                                </span>
                                <div class="h-px bg-slate-200 dark:bg-slate-700 flex-1"></div>
                            </div>
                        <?php endif; ?>
                ...
                <div class="p-4 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-700">
                    <form id="comment-form" class="flex gap-4">
                        ...
                        <textarea name="comment" rows="2" placeholder="Escribe un comentario..." class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none dark:text-white resize-none" required></textarea>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 rounded-lg font-bold transition shadow-md self-end h-10 flex items-center"><i class="fas fa-paper-plane"></i></button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-darkcard rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 p-6">
                <h3 class="font-bold text-slate-700 dark:text-white mb-4 uppercase text-xs tracking-wider border-b border-slate-100 dark:border-slate-700 pb-2">Gestión del Ticket</h3>
                <form id="manage-form" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="estado" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:ring-blue-500 p-2.5 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                    ...
                    <div>
                        ...
                        <select name="prioridad" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <select name="agente_id" id="agente-select" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                                <option value="">-- Sin Asignar --</option>
                                <?php foreach($agentesList as $ag): ?>
                                    <option value="<?php echo $ag['id']; ?>" <?php echo $ag['id'] == $ticket['agente_actual_id'] ? 'selected' : ''; ?>><?php echo $ag['nombre']; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <button type="button" onclick="assignToMe()" class="bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 p-2.5 rounded-lg transition" title="Tomar Ticket"><i class="fas fa-hand-paper"></i></button>
                        </div>
                    </div>
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-lg shadow-md transition mt-2">Guardar Cambios</button>
                </form>
            </div>
            <?php endif; ?>
            ...
            <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
                <?php if ($isStaff): ?>
                    <h3 class="font-bold text-slate-700 dark:text-white mb-6 uppercase text-xs tracking-wider border-b border-slate-100 dark:border-slate-700 pb-2">Información Solicitante</h3>
                    <div class="flex flex-col items-center mb-6">
                        <?php 
                            $cAvatar = $ticket['c_avatar'] ? "/HelpDesk/assets/img/avatars/".$ticket['c_avatar'] : "https://api.dicebear.com/7.x/avataaars/svg?seed=".$ticket['c_n1'];
                            $cNombre = buildName($ticket['c_n1'], $ticket['c_n2'], $ticket['c_a1'], $ticket['c_a2']);
                        ?>
                        <img src="<?php echo $cAvatar; ?>" class="w-20 h-20 rounded-full border-4 border-slate-50 dark:border-slate-700 mb-2 object-cover">
                ...
                <?php else: ?>
                    <h3 class="font-bold text-slate-700 dark:text-white mb-6 uppercase text-xs tracking-wider border-b border-slate-100 dark:border-slate-700 pb-2">Agente Asignado</h3>
                    <?php if($ticket['agente_actual_id']): ?>
                        <?php 
                            $aAvatar = $ticket['a_avatar'] ? "/HelpDesk/assets/img/avatars/".$ticket['a_avatar'] : "https://api.dicebear.com/7.x/avataaars/svg?seed=".$ticket['a_n1'];
                            $aNombre = buildName($ticket['a_n1'], $ticket['a_n2'], $ticket['a_a1'], $ticket['a_a2']);
                        ?>
                        <div class="flex flex-col items-center mb-6">
                            <img src="<?php echo $aAvatar; ?>" class="w-20 h-20 rounded-full border-4 border-slate-50 dark:border-slate-700 mb-2 object-cover">
                    ...
                    <?php else: ?>
                        <div class="text-center py-8 text-slate-400"><div class="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-3"><i class="fas fa-user-clock text-3xl"></i></div><p class="text-sm font-medium">Buscando agente disponible...</p></div>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        // Inicializar el visor profesional de evidencias con botones avanzados
    ...
    <script>
        ...
        async function handleFormSubmit(formId) {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                const minimumWaitTime = new Promise(resolve => setTimeout(resolve, 3000));
                const fetchRequest = fetch('/Helpdesk/db/public/tickets/update-ticket-action.php', { method: 'POST', body: formData });

                try {
                    const [_, res] = await Promise.all([minimumWaitTime, fetchRequest]);
                    ...
                    try {
                        ...
                        console.error(text); 
                        Swal.fire('Error del Servidor', 'Revisa la consola para más detalles', 'error'); 
                    }
                } catch (error) { 
                    console.error(error); 
